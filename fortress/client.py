...
def intro():
    ...
    while menu:
        ...
        Button(mainmenu_start, 680, 550, 300, 50, mainmenu_start_click, 680, 550, client)
        pygame.display.update()
        clock.tick(7)

# 준비 화면
def ready():
    global gameDisplay, player1, player2, win, defeated, turn, environment, start_cnt
    # 초기화
    player1, player2, win, defeated, environment = None, None, None, None, None
    menu = True
    turn = 1
...
# 클라이언트 관리
def client():
    global player1, player2, start_cnt
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client_socket.connect((HOST, PORT))
    disp = Thread(target=ready)
    disp.start()
    data = client_socket.recv(1024).decode('utf-8') # tjqj한테 시작신호 받기
    if "시작" in data: # 넘어갈 수 있게 
        start_cnt = 1 # 1일 경우 시작
